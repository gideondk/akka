...
class RemoteConfigSpec extends AkkaSpec(
  ...
  "Remoting" must {
    ...
    "contain correct configuration values in reference.conf" in {
      ...
      WatchFailureDetectorConfig.getDouble("threshold") must be(8.0 plusOrMinus 0.0001)
      WatchFailureDetectorConfig.getInt("max-sample-size") must be(200)
      Duration(WatchFailureDetectorConfig.getMilliseconds("acceptable-heartbeat-pause"), MILLISECONDS) must be(3 seconds)
