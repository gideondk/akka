...
class ClusterMessageSerializerSpec extends AkkaSpec {
  ...
  "ClusterMessages" must {
    ...
    "be serializable" in {
      ...
      val uniqueAddress = UniqueAddress(Address("akka.tcp", "system", "some.host.org", 4711), 17)
      ...
      checkSerialization(GossipEnvelope(a1.uniqueAddress, g2.copy(overview = g2.overview.copy(unreachable = Set(e1, f1)))))

      checkSerialization(InternalClusterAction.Welcome(uniqueAddress, g2))

      val mg = MetricsGossip(Set(NodeMetrics(a1.addressFIXME, 4711, Set(Metric("foo", 1.2, None))),
        NodeMetrics(b1.addressFIXME, 4712, Set(Metric("foo", 2.1, Some(EWMA(value = 100.0, alpha = 0.18))),
      ...
      checkSerialization(MetricsGossipEnvelope(a1.addressFIXME, mg, true))
    }
  }
}
