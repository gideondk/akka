...
abstract class RemoteNodeDeathWatchSpec
  ...
  "RemoteNodeDeathWatch (" + scenario + ")" must {
    ...
    "receive Terminated when remote actor is stopped" taggedAs LongRunningTest in {
      ...
      runOn(second) {
        val subject = system.actorOf(Props(new ProbeActor(testActor)), "subject1")
        enterBarrier("actors-started-1")
    ...
    "cleanup after watch/unwatch" taggedAs LongRunningTest in {
      runOn(first) {
        val watcher = system.actorOf(Props(new ProbeActor(testActor)), "watcher2")
      ...
      runOn(second) {
        system.actorOf(Props(new ProbeActor(testActor)), "subject2")
        enterBarrier("actors-started-2")
        enterBarrier("watch-2")
        enterBarrier("unwatch-2")
      }
    ...
    "cleanup after bi-directional watch/unwatch" taggedAs LongRunningTest in {
      val watcher = system.actorOf(Props(new ProbeActor(testActor)), "watcher3")
      system.actorOf(Props(new ProbeActor(testActor)), "subject3")
    ...
    "cleanup after bi-directional watch/stop/unwatch" taggedAs LongRunningTest in {
      val watcher1 = system.actorOf(Props(new ProbeActor(testActor)), "w1")
      val watcher2 = system.actorOf(Props(new ProbeActor(testActor)), "w2")
      system.actorOf(Props(new ProbeActor(testActor)), "s1")
      val s2 = system.actorOf(Props(new ProbeActor(testActor)), "s2")
      enterBarrier("actors-started-3")
    ...
    "cleanup after stop" taggedAs LongRunningTest in {
      runOn(first) {
        ...
        val a1 = system.actorOf(Props(new ProbeActor(p1.ref)), "a1")
        val a2 = system.actorOf(Props(new ProbeActor(p2.ref)), "a2")
        val a3 = system.actorOf(Props(new ProbeActor(p3.ref)), "a3")
      ...
      runOn(second) {
        val p1, p2, p3 = TestProbe()
        val b1 = system.actorOf(Props(new ProbeActor(p1.ref)), "b1")
        val b2 = system.actorOf(Props(new ProbeActor(p2.ref)), "b2")
        val b3 = system.actorOf(Props(new ProbeActor(p3.ref)), "b3")
        enterBarrier("actors-started-5")
        ...
        p1.receiveN(2, 5 seconds).collect { case t: Terminated ⇒ t.actor }.toSet must be(Set(a1, a2))
        p3.expectTerminated(a3, 5 seconds)
    ...
    "receive Terminated when watched node is shutdown" taggedAs LongRunningTest in {
      runOn(first) {
        val watcher = system.actorOf(Props(new ProbeActor(testActor)), "watcher6")
        val watcher2 = system.actorOf(Props(new ProbeActor(system.deadLetters)))
      ...
      runOn(second) {
        system.actorOf(Props(new ProbeActor(testActor)), "subject6")
        enterBarrier("actors-started-6")
